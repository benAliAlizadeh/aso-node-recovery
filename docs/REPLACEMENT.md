...
6. after the provider reports `RUNNING`, persist that state and wait `ASO_REPLACEMENT_VPS_BOOT_GRACE_SECONDS` (default `180`); then run five verification rounds one minute apart. Each round first checks the VPS from outside Iran and only treats an Iran failure as filtering when outside-Iran reachability is confirmed.
